...
pulls live and historical prices from free market-data providers (Kraken, Bitvavo, CoinGecko,
Twelve Data, the ECB and Yahoo Finance), computes portfolio value and gains, and stores
...
|----------|----------------|
| [architecture.md](architecture.md) | Technical architecture: the two-container topology, request flow, backend layering, the EF Core entity/schema list, cookie authentication + DataProtection, the snake_case JSON contract, the market-data providers and price cache, the holdings/wealth calculation rules, and the project layout. |
...
| [csv-import.md](csv-import.md) | The CSV import feature: auto-detection, idempotent re-imports (import keys), Check & Import, plus the recognized headers, an example file, and the row→transaction mapping for each of the four supported formats (revolut-stocks, revolut-commodities, trezor, generic). |
| [development.md](development.md) | Local development: prerequisites, repo layout, building with `dotnet build`, the server and client test suites, the import-verification and timing tools, running each project standalone, running via Docker, and the EF Core `EnsureCreated` approach. |
