...
- Set `CAPITRACK_INIT_PASSWORD` (or use the auto-generated first-run password from the logs) and change it after first login; no password is stored in the source tree
- Authentication cookies are protected by ASP.NET Core DataProtection keys, persisted to the data volume
