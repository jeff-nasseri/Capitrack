...
public partial class ImporterService(CapitrackDbContext db)
{
    private static readonly string[] ValidTypes =
        ["buy", "sell", "transfer_in", "transfer_out", "dividend", "interest", "fee"];

    public record ImportedTransaction(string Symbol, string Type, double Quantity, double Price, double Fee, string Currency, string Date, string Notes);

    // ---- CSV parsing into header->value dictionaries (csv-parse columns:true equivalent) ----
    public static (List<Dictionary<string, string>> Records, List<string> Headers) ParseCsv(string content)
    {
    ...
    public (string Format, List<string> Headers) Detect(string content)
    {
        var (records, headers) = ParseCsv(content);
        if (records.Count == 0) return ("unknown", []);
        return (DetectFormat(headers), headers);
    }
    ...
    private HashSet<string> ExistingFingerprints(int accountId)
    {
        var set = new HashSet<string>();
        foreach (var t in db.Transactions.Where(t => t.AccountId == accountId))
            set.Add(Fingerprint(t.AccountId, t.Symbol, t.Type, t.Quantity, t.Price, t.Date));
        return set;
    }

    // ---- Main import ----
    public ImportResultDto ImportCsv(string content, int accountId, string? formatHint)
    {
        var (records, headers) = ParseCsv(content);
        ...
        var existing = ExistingFingerprints(accountId);
        ...
        for (var i = 0; i < parsed.Count; i++)
        {
            ...
            try
            {
                ...
                {
                    AccountId = accountId, Symbol = tx.Symbol, Type = tx.Type, Quantity = tx.Quantity,
                    Price = tx.Price, Fee = tx.Fee, Currency = tx.Currency, Date = tx.Date, Notes = tx.Notes
                });
                existing.Add(fp);
                imported++;
            }
